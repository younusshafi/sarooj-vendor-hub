...
7b. ~~MR → link-first bidding (n8n)~~ — ✅ **DONE 2026-06-28 (WF8 surgical edit via REST API; backed up first).** Finding: WF8 already appended the `/bid/<token>` link; the edit makes it a **prominent top CTA** and replaces the "reply directly to this email" instruction with link guidance. Canonical vendor URL: `https://procurement.scc.zavia-ai.com`. Edit preserved all 19 nodes + webhookId; WF8 stays active. **VERIFY: do one Test-Mode dispatch from the app to confirm WF8 still fires + the email is link-first;** if the webhook 404s (unlikely for an edited existing wf), toggle WF8 once in the n8n UI. Rollback = `docs/backups/n8n/IDeJjPRijKAUjVXU.*.json`. Optional FE follow-up: demote the "Original Email" panel on Bid Review for portal bids. See memory `mr-bid-model-link-first`.
