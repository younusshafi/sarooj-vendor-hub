...
> ⚠️ **VENDOR TABLE IS IN TEST-ISOLATION (applied 2026-06-28).** 1,405 real vendors set to `status='blacklisted'`; only the 10 `SCC TEST —` vendors are `listed`/active, so the working set = 10 safe test vendors (sarooj.com emails). **MUST RESTORE before go-live.** Reversal:
> `update scc_procurement.vendors v set status=b.status, duplicate_flag=b.duplicate_flag from scc_procurement.vendors_status_backup_20260628 b where v.vendor_id=b.vendor_id;` then `drop table scc_procurement.vendors_status_backup_20260628;`
> Caveat: the 10 test vendors only carry category `TEST_ALWAYS`, so category-based auto-match (WF7) won't surface them for real categories — add real categories to them if you need auto-match (manual add-by-name works).
...
5. ~~Dispatch~~ — ✅ **DONE 2026-06-28.** SR Vendors tab → "Vendor bid links" panel has an **"Email all vendors"** button: emails each invited vendor their `/sr-bid/<token>` link via the generic WF15 webhook, stamps `sent_at`/`status='sent'`. **Honors Dispatch Test Mode** (when ON, sends to the officer instead of real vendors). Manual copy/open still available per vendor.

**Vendor data (PLANNED, not started)**
V1. **Enrich `vendors` from `Procurement/vendor data from SCC 21 jun 2026/`** — back up the table first; fill CR/VAT/legal/certs + website/sector/products on existing vendors; add ~284 new vendors; populate categories from the taxonomy. Reconcile the `CAT-###` taxonomy with the app's existing `categories`/`category_groups` first; review ~91 fuzzy matches. See memory `vendor-enrichment-and-test-data`.
V2. **Test-data safety** — make `dispatch_test_mode` honored on EVERY send path (WF8 ignores it today; SR dispatch honors it) + a curated TEST-vendor set (controlled inboxes, all categories) for dispatch + outreach. Recommended over scrubbing real emails. Decision pending.

**Notifications & negotiation (n8n — affects MR + SR)**
6. Notification emails (n8n). **n8n is now editable via the public REST API** with the key in `Procurement/.env` (`N8N_API_KEY`/`N8N_BASE_URL`) — the claude.ai n8n MCP is still expired/read-only, but the REST API works. Recipe for NEW webhook workflows: include a `webhookId` on the webhook node, then activate (API activation alone won't register the webhook without it). All SCC workflows backed up to `docs/backups/n8n/` (gitignored) before any change.
   - ✅ **email Rabia the comparison review link — DONE 2026-06-28.** Created+activated **"SCC - WF15 Approval Email"** (generic Webhook `scc-approval-email` → Gmail "sarooj procurement gmail" → Respond), tested e2e. `system_settings.approver_email` added. Frontend `src/lib/notify.ts` (`sendApprovalEmail`/`getApproverEmail`) called from MR comparison + SR panel on submit. WF15 is a generic to/subject/text sender — reuse it for the rest.
   - ⏳ still pending: notify officer on vendor submit · notify officer on approve/return/revoke (can reuse WF15 via frontend calls; need an officer-email source, e.g. `system_settings.officer_emails` or rfq.created_by).
7. Materials negotiation / add-vendor-to-issued-RFQ — awaiting procurement sign-off.
7b. ~~MR → link-first bidding (n8n)~~ — ✅ **DONE 2026-06-28 (WF8 surgical edit via REST API; backed up first).** Finding: WF8 already appended the `/bid/<token>` link; the edit makes it a **prominent top CTA**, replaces the "reply directly to this email" instruction with link guidance, AND **strips the inline schedule `<table>`** (replaced with a "see the link" note) → the email is now **link-only**, no item table. Canonical vendor URL: `https://procurement.scc.zavia-ai.com`. Edit preserved all 19 nodes + webhookId; WF8 stays active. **VERIFY: do one Test-Mode dispatch from the app to confirm WF8 still fires + the email is link-first;** if the webhook 404s (unlikely for an edited existing wf), toggle WF8 once in the n8n UI. Rollback = `docs/backups/n8n/IDeJjPRijKAUjVXU.*.json`. Optional FE follow-up: demote the "Original Email" panel on Bid Review for portal bids. See memory `mr-bid-model-link-first`.

**Your-action / go-live checks**
8. Live WF7 generation test (SAP upload → generate) — the one unverified n8n change.
9. Settings → Dispatch Test Mode → OFF at go-live. ⚠️ **GAP found 2026-06-28: WF8 (materials dispatch) does NOT read `dispatch_test_mode`** — it always sends to the actual `rfq_vendors`, so the toggle currently does nothing for MR sends. (SR's new "Email all vendors" DOES honor it, frontend-side.) Before go-live, either make WF8 honor test mode or rely on vendor selection. Worth fixing for MR.
