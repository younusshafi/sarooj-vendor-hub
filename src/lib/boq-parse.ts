...
export async function parseBoqPdf(file: File): Promise<ParseResult> {
  ...
  for (let p = 1; p <= doc.numPages; p++) {
    ...
    for (const item of content.items) {
      if ("str" in item && typeof item.str === "string") {
        allTextItems.push({
          str: item.str,
          transform: (item as { transform?: number[] }).transform ?? [1, 0, 0, 1, 0, 0],
        });
      }
    }
  }
  ...
  if (lines.length === 0) {
    return {
      ...
      sourceKind: "manual",
      source_confidence: "manual",
      source_remark: "Could not extract structured item rows from the PDF. Enter lines manually.",
    };
  }
