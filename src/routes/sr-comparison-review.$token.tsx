...
import {
  srComparisonGetByToken,
  srComparisonDecide,
  type SrComparisonReview,
} from "@/lib/sr-comparison";
import { fmtOmr as fmt } from "@/lib/omr";
import { notifyDecision } from "@/lib/notify";

export const Route = createFileRoute("/sr-comparison-review/$token")({
  head: () => ({
    meta: [{ title: "Subcontract comparison approval — Sarooj Construction Company" }],
  }),
  component: SrReviewPage,
});

function SrReviewPage() {
  const { token } = Route.useParams();
  const [data, setData] = useState<SrComparisonReview | null>(null);
  ...
  const view = useMemo(() => {
    ...
    const descIdx = data.descIdx;
    const eqMap = new Map<string, number>();
    for (const e of data.equalizations)
      eqMap.set(`${e.line_id}::${e.rfq_vendor_id}`, e.adjustment_omr);
    const rows = data.awards.map((a) => {
      const line = data.lines.find((l) => l.line_id === a.line_id);
      const vendor = data.vendors.find((v) => v.rfq_vendor_id === a.rfq_vendor_id);
      const vline = vendor?.lines.find((l) => l.line_id === a.line_id);
      const qty = line?.qty ?? 0;
      const rate = vline?.unit_rate_omr ?? null;
      const eq = eqMap.get(`${a.line_id}::${a.rfq_vendor_id}`) ?? 0;
      const raw = rate != null ? (vline?.amount_omr ?? rate * qty) : null;
      const equalized = raw != null ? raw + eq : null;
      const desc =
        descIdx >= 0 && line?.cells[descIdx]
          ? line.cells[descIdx]
          : (line?.cells.find((c) => c && c.trim()) ?? "—");
      return {
        line_id: a.line_id,
        seq: line?.seq ?? 0,
        description: desc,
  ...
  const decide = async (decision: "approve" | "return" | "revoke") => {
    ...
    setBusy(true);
    try {
      const res = await srComparisonDecide(token, decision, notes);
      if (res.ok) {
        setDone(decision);
        if (data && data.found) {
          notifyDecision({
      ...
      <header className="w-full bg-header text-header-foreground">
        <div className="mx-auto flex h-16 max-w-[1000px] items-center justify-between px-6 md:px-10">
          ...
          <span className="text-[13px]" style={{ color: "var(--sidebar-foreground)" }}>
            Subcontract Comparison Approval
          </span>
        </div>
      </header>
          ...
          <>
            <div className="mb-6 rounded-xl border border-border bg-card p-6">
              <h1 className="font-serif text-[24px] leading-tight text-foreground">
                {data.rfq.rfq_reference}
                {data.rfq.title ? ` — ${data.rfq.title}` : ""}
              </h1>
              <p className="mt-1 text-[14px] text-muted-foreground">
                {data.rfq.scope ?? data.rfq.project_name ?? ""}
            ...
            <div className="rounded-xl border border-border bg-card p-6">
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {view.rows.map((r) => (
                      <tr key={r.line_id} className="border-t border-border align-top">
                        <td className="py-2 pr-3 text-xs text-muted-foreground">{r.seq}</td>
              ...
              <div className="mt-5 flex flex-wrap items-end justify-between gap-4">
                ...
                <div className="text-right">
                  ...
                  <div className="text-xl font-bold" style={{ color: "#0D5C3A" }}>
                    {fmt(view.grand)} OMR
                  </div>
