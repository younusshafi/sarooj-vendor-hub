...
1. **Frontend-first; backend allowed with care (the user IS the operator).** `younus@zavia-ai.com` owns both the frontend and the backend (Supabase + n8n), so backend changes ARE permitted when a task needs them — you no longer "stop and hand off." Guardrails: **(a) keep it ADDITIVE** — new tables/RPCs/workflows; do not alter or drop existing schema/workflows unless explicitly asked, and then surgically. **(b) BACK UP first** — export the n8n workflow JSON before editing it (`docs/backups/n8n/`, gitignored) and save a reference copy of any applied SQL to `docs/applied-migrations/`. **(c) Confirm before destructive or live-production edits.** **(d) Mirror existing patterns.** Tools: Supabase via MCP `apply_migration`/`execute_sql`; n8n via the **public REST API** (`N8N_API_KEY`/`N8N_BASE_URL` in `Procurement/.env` — never `source` that file, grep the values out). n8n is a **SHARED multi-client instance** — touch only `SCC-*` workflows unless told otherwise. New n8n webhook workflows need a `webhookId` on the webhook node to register. Still prefer a frontend solution when one exists; reach for backend only when the task genuinely needs it.
